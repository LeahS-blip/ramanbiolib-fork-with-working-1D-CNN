...
SUMMARY_DIR = Path('outputs/comparisons/20260429_softgate_threshold_sweep')
...
for tau in THRESHOLDS:
    ...
    model_cfg = json.loads(Path('outputs/model/model_config.json').read_text(encoding='utf-8'))
    ...
    shutil.copy2('outputs/model/model_config.json', run_dir / f'{run_prefix}_model_config.json')
    ...
    fig.suptitle(f'p_PM distributions by true class, tau={tau:.2f}', fontsize=13)
    fig.tight_layout()
    ...
    print(
        f"tau {tau:.2f}: acc={summary_rows[-1]['accuracy']:.4f} macro_f1={summary_rows[-1]['macro_f1']:.4f} "
...
for _, row in summary_df.iterrows():
    print(
        f"tau={row['tau']:.2f} acc={row['accuracy']:.4f} macro_f1={row['macro_f1']:.4f} "
